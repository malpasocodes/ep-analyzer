# EP Analyzer: Program-Level Data
...
A dashboard showing the full landscape: 6,275 institutions across 50 states and DC, with risk distributions at both the institution and program level. This is your entry point for understanding the scale of the problem.

### 2. States

Browse all 50 states and DC and their EP thresholds — the statewide median earnings for high school graduates that each institution must beat. Thresholds vary significantly by state, which is the core of the geographic bias problem. Drill into any state to see its institutions ranked by earnings margin, with risk donut charts and margin histograms.

### 3. Institutions

Search and filter 6,275 institutions by name, state, or risk level. Each institution detail page shows:
...
Among programs with reported earnings, those most likely to fail the EP test read like a list of vocational and creative fields:
...
*Counts reflect programs with reported earnings that fail the EP test. Including Monte Carlo estimates for suppressed programs increases these figures substantially (e.g., Cosmetology rises to 1,648).*

Cosmetology alone accounts for over 12% of all high-risk programs with reported earnings. These are predominantly certificate and associate-level programs at community colleges and for-profit schools — institutions that serve students who often have the fewest alternatives.
...
Of 6,275 institutions (50 states and DC):

- **1,264** (20.1%) are High Risk — currently failing
- **885** (14.1%) are Moderate Risk — vulnerable
- **875** (13.9%) have no reportable data at all

That's a third of all institutions either failing or too close to the line for comfort, and another seventh completely invisible to the metric.
